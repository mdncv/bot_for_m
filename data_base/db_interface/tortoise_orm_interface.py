from tortoise import run_async
from tortoise.contrib.sqlite.functions import Random

from data_base.models import UserTable, DictTable, Cheers


# Working with user table

async def user_table_create_entry(user_id: int) -> None:

    defaults = dict(poll_id=0, correct_answer_id=0, word='', correct_answers=0, incorrect_answers=0, learning_lang=32,
                    native_lang=1)
    await UserTable.update_or_create(user_id=user_id, defaults=defaults)


async def user_table_check_entry(user_id: int) -> bool:
    return await UserTable.filter(user_id=user_id).exists()


async def user_table_read_entry(poll_id: int) -> dict:

    user = await UserTable.filter(poll_id=poll_id).first()

    return dict(user_id=user.user_id, poll_id=user.poll_id, correct_option=user.correct_answer_id, word=user.word,
                learning_lang=user.learning_lang, native_lang=user.native_lang)


async def user_table_update_languages(user_id: int, learning_lang: int, native_lang: int) -> None:
    await UserTable.filter(user_id=user_id).update(learning_lang=learning_lang, native_lang=native_lang)


async def user_table_add_poll(user_id: int, poll_id: int, correct_option_id: int, word: str) -> None:
    await UserTable.filter(user_id=user_id).update(poll_id=poll_id, correct_answer_id=correct_option_id, word=word)


async def user_table_check_poll_existence(poll_id: int) -> bool:
    return await UserTable.filter(poll_id=poll_id).exists()


async def user_table_close_poll(user_id: int) -> None:
    await UserTable.filter(user_id=user_id).update(poll_id=0, correct_answer_id=0, word='')


# Working with stats table

async def user_table_increase_stats(user_id: int) -> None:

    user = await UserTable.filter(user_id=user_id).first()

    await UserTable.filter(user_id=user_id).update(correct_answers=user.correct_answers + 1)


async def user_table_decrease_stats(user_id: int) -> None:

    user = await UserTable.filter(user_id=user_id).first()

    await UserTable.filter(user_id=user_id).update(incorrect_answers=user.incorrect_answers + 1)


async def user_table_read_stats(user_id: int) -> tuple:

    user = await UserTable.filter(user_id=user_id).first()

    return user.correct_answers, user.incorrect_answers


# Working with dictionary table

async def dictionary_table_get_size(user_id: int) -> int:
    return await DictTable.filter(user_id=user_id).count()


async def dictionary_table_add_entry(word: str, description: str, user_id: int) -> None:
    await DictTable.create(user_id=user_id, word=word, description=description)


async def dictionary_table_check_entry_existence(word: str, user_id: int) -> bool:
    return await DictTable.filter(word=word, user_id=user_id).exists()


async def dictionary_table_read_entries(user_id: int) -> list:

    words = await DictTable.filter(user_id=user_id).annotate(order=Random()).order_by('order').limit(4)

    return [(word.word, word.description) for word in words]


async def dictionary_table_delete_entry(word_to_remove: str, user_id: int) -> None:
    await DictTable.filter(word=word_to_remove, user_id=user_id).delete()


async def dictionary_table_delete_user_entries(user_id: int) -> None:
    await DictTable.filter(user_id=user_id).delete()


# Working with cheers table

async def cheers_table_add_entry(cheer: str, not_cheer: str) -> None:
    await Cheers.get_or_create(cheer=cheer, not_cheer=not_cheer)


async def cheers_table_get_entry(regime: bool) -> str:
    cheer = await Cheers.annotate(order=Random()).order_by('order').first()
    return cheer.cheer if regime else cheer.not_cheer


async def main():
    pass


if __name__ == '__main__':

    run_async(main())
